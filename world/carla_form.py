...
class CarlaConfig:
    ...
    log_csv: bool = True
    # --- Mode conduite libre (nouveau) ---
    mode_conduite: str = "scenario"        # "scenario" | "libre"
    n_vehicules_ia: int = 30               # nombre de véhicules IA
    n_pietons_ia: int = 15                 # nombre de piétons IA
    assistance: str = "alerte"             # "eval" | "alerte" | "aeb"
...
class FormulaireCarla:
    ...
    def _build(self):
        ...
        # --- Onglet 3 : Conduite libre ---
        f_libre = ttk.Frame(notebook, padding=12)
        notebook.add(f_libre, text="Conduite libre")

        ttk.Label(f_libre, text="Activer le mode conduite libre",
                  font=("", 10, "bold")).grid(row=0, column=0, columnspan=2, sticky="w", pady=(0, 6))
        ttk.Label(f_libre, text="Vous conduisez l'ego au clavier, le monde CARLA vit tout\n"
                                "seul (véhicules IA + piétons), le moteur observe et affiche.",
                  foreground="#555").grid(row=1, columnspan=2, sticky="w", pady=(0, 10))

        self.mode_libre = tk.BooleanVar(value=(self.cfg.mode_conduite == "libre"))
        ttk.Checkbutton(f_libre, text="Mode conduite libre",
                        variable=self.mode_libre).grid(row=2, columnspan=2, sticky="w", pady=6)

        ttk.Separator(f_libre).grid(row=3, columnspan=2, sticky="ew", pady=8)

        ttk.Label(f_libre, text="Densité de trafic",
                  font=("", 10, "bold")).grid(row=4, columnspan=2, sticky="w")
        ttk.Label(f_libre, text="Véhicules IA :").grid(row=5, column=0, sticky="w")
        self.n_veh = tk.IntVar(value=self.cfg.n_vehicules_ia)
        ttk.Spinbox(f_libre, from_=0, to=100, textvariable=self.n_veh, width=8).grid(row=5, column=1, sticky="w")
        ttk.Label(f_libre, text="Piétons IA :").grid(row=6, column=0, sticky="w")
        self.n_pi = tk.IntVar(value=self.cfg.n_pietons_ia)
        ttk.Spinbox(f_libre, from_=0, to=50, textvariable=self.n_pi, width=8).grid(row=6, column=1, sticky="w")

        ttk.Separator(f_libre).grid(row=7, columnspan=2, sticky="ew", pady=8)

        ttk.Label(f_libre, text="Assistance du moteur",
                  font=("", 10, "bold")).grid(row=8, columnspan=2, sticky="w")
        self.assist = tk.StringVar(value=self.cfg.assistance)
        ttk.Radiobutton(f_libre, text="Évaluation seule (le moteur observe)",
                         variable=self.assist, value="eval").grid(row=9, columnspan=2, sticky="w")
        ttk.Radiobutton(f_libre, text="Alerte visuelle en DANGER / CRITICAL",
                         variable=self.assist, value="alerte").grid(row=10, columnspan=2, sticky="w")
        ttk.Radiobutton(f_libre, text="Freinage automatique d'urgence (AEB)",
                         variable=self.assist, value="aeb").grid(row=11, columnspan=2, sticky="w")

        ttk.Separator(f_libre).grid(row=12, columnspan=2, sticky="ew", pady=8)

        ttk.Label(f_libre, text="Perception (mode oracle)",
                  font=("", 10, "bold")).grid(row=13, columnspan=2, sticky="w")
        ttk.Label(f_libre, text="Rayon de perception (m) :").grid(row=14, column=0, sticky="w")
        self.rayon = tk.DoubleVar(value=self.cfg.rayon_perception_m)
        ttk.Entry(f_libre, textvariable=self.rayon, width=8).grid(row=14, column=1, sticky="w")
        ttk.Label(f_libre, text="Demi-angle du cône (°) :").grid(row=15, column=0, sticky="w")
        self.demi_angle = tk.DoubleVar(value=self.cfg.demi_angle_perception_deg)
        ttk.Entry(f_libre, textvariable=self.demi_angle, width=8).grid(row=15, column=1, sticky="w")
        ...
        ttk.Label(f3, text="(ignoré en mode conduite libre)", foreground="#888").grid(row=0, column=1, sticky="w")
    ...
    def _lancer(self):
        cfg = CarlaConfig(
            ...
            mode_conduite=("libre" if self.mode_libre.get() else "scenario"),
            n_vehicules_ia=self.n_veh.get(),
            n_pietons_ia=self.n_pi.get(),
            assistance=self.assist.get(),
            rayon_perception_m=self.rayon.get(),
            demi_angle_perception_deg=self.demi_angle.get(),
        )
