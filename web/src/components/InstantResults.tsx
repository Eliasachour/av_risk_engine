...
export function InstantResults({ result, loading }: Props) {
  ...
  return (
    <div className="panel">
      ...
      {/* Consigne d'action : ce qu'il faudrait faire pour revenir en SAFE.
          Le chiffrage vient de l'inversion du moteur (dichotomie sur la vitesse
          ego), il est donc cohérent avec le verdict par construction. */}
      {result.recommandation && (
        <div className={`reco reco-${result.recommandation.urgence}`}>
          <div className="reco-head">
            <span className="reco-pastille" />
            <span className="reco-message">{result.recommandation.message}</span>
          </div>
          <div className="reco-why">{result.recommandation.justification}</div>
          {result.recommandation.vitesse_cible_kmh != null &&
           result.recommandation.delta_vitesse_kmh != null &&
           result.recommandation.delta_vitesse_kmh > 0.5 && (
            <div className="reco-chiffres">
              <span>
                <b>{result.recommandation.delta_vitesse_kmh.toFixed(0)}</b> km/h à retirer
              </span>
              <span>
                cible <b>{result.recommandation.vitesse_cible_kmh.toFixed(0)}</b> km/h
              </span>
              {result.recommandation.distance_sure_m != null && (
                <span>
                  ou <b>{result.recommandation.distance_sure_m.toFixed(0)}</b> m de distance
                </span>
              )}
            </div>
          )}
