...
/** Consigne d'action produite par le moteur (risk_engine/advice.py).
 *  `urgence` : 0 aucune · 1 anticiper · 2 agir · 3 agir immédiatement. */
export interface Recommandation {
  action: string;
  message: string;
  justification: string;
  urgence: number;
  delta_vitesse_kmh: number | null;
  vitesse_cible_kmh: number | null;
  distance_sure_m: number | null;
}
