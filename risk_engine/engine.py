...
class RiskConfig:
    ...
    # PET géométrique 2D (D15). À False, on retombe sur l'estimation
    # « snapshot » historique (distance / v_ego), conservée pour comparaison.
    pet_2d: bool = True
    # Exclure le TTC du classement pour les conflits de CROISEMENT, où il est
    # hors de son domaine de validité (Westhofen et al.). N'est SÛR qu'associé
    # à `pet_2d` : seul, il produit 4 détections manquées (mesuré, voir D15).
    ttc_hors_croisement: bool = True
    #: Horizon de confiance du PET 2D (s) — au-delà, PET non renseigné.
    pet_horizon_s: float = 4.0
    #: Nombre de crans de rétrogradation autorisés par le filtrage latéral.
    #: 2 par défaut (marge de sécurité) ; 3 autorise la descente jusqu'à SAFE,
    #: à réserver aux cas où l'écart latéral vient des identifiants de voie
    #: CARLA et non d'une estimation euclidienne. Voir D20.
    retrogradation_crans: int = 2
    pet_safe: float = 3.0
    pet_watch: float = 1.5
...
def assess_agent(ctx: ScenarioContext, agent: Agent, cfg: RiskConfig) -> AgentRisk:
    ...
    if cfg.pet_2d:
        _pet = metrics.pet_2d(v_ego, v_ag, d, cap, agent.ecart_lateral_m,
                              horizon_s=cfg.pet_horizon_s)
    else:
    ...
    # Le RSS suppose une décélération garantie `b_min` pour l'ego. Cette
    # décélération ne peut PAS dépasser l'adhérence réellement disponible : sur
    # verglas (µ·g ≈ 2,9 m/s²), promettre 4 m/s² est physiquement faux et
    # sous-estime la distance de sécurité — précisément là où il faudrait être
    # le plus prudent. On plafonne donc b_min à µ·g.
    #
    # `b_max` (décélération maximale supposée du véhicule de tête) n'est
    # volontairement PAS plafonnée : c'est une hypothèse *pessimiste* sur le
    # comportement de l'autre, et la relâcher ferait BAISSER la distance de
    # sécurité exigée sur sol dégradé — l'inverse de ce qu'un système de
    # sécurité doit faire. Choix conservateur assumé, voir DECISIONS.md (D14).
    b_min_eff = min(cfg.b_min, a_max)
    rss = metrics.rss_min_distance(v_ego, v_ag, cfg.rho, cfg.a_accel, b_min_eff, cfg.b_max)
    ...
    # Le TTC suppose un rapprochement colinéaire : hors de ce cas (croisement),
    # il mesure une grandeur sans signification physique (Westhofen et al.).
    # Option D15 : le neutraliser pour les croisements et laisser le PET
    # géométrique porter l'évaluation temporelle. L'effet est mesuré.
    croisement = modifiers.famille_conflit(cap) is modifiers.FamilleConflit.CROISEMENT
    if cfg.ttc_hors_croisement and croisement:
        n_ttc = RiskLevel.SAFE
    n_thw = _band_haut(_thw, cfg.thw_safe, cfg.thw_watch, cfg.thw_danger)
    # Le PET est lui aussi divisé par la marge contextuelle. C'était inutile
    # tant que le PET restait secondaire, mais il devient la métrique temporelle
    # décisive en croisement (D15) : sans cela, la vulnérabilité de l'usager
    # cesserait d'agir exactement là où elle importe le plus — un piéton et une
    # voiture en géométrie identique obtiendraient le même niveau.
    pet_eff = _pet / marge if math.isfinite(_pet) else math.inf
    n_pet = _band_haut(pet_eff, cfg.pet_safe, cfg.pet_watch, cfg.pet_danger)
    ...
    # Un VRU qui TRAVERSE n'est jamais rétrogradé : la prudence maximale sur
    # l'usager vulnérable prime sur la réduction des fausses alarmes. Mesure à
    # l'appui — SC-21 (piéton de nuit, CRITICAL) a un facteur de menace de 0,00
    # et serait rétrogradé, donc manqué.
    #
    # En revanche, un VRU qui LONGE la route (mouvement parallèle, sans
    # composante transversale) n'est pas sur une trajectoire de collision : il
    # peut être rétrogradé comme n'importe quel agent. Sans cette distinction,
    # tout piéton marchant sur un trottoir produisait un DANGER permanent en
    # conduite libre — le moteur devenait inutilisable hors scénario scripté.
    vru_traverse = est_vru and (
        modifiers.famille_conflit(cap) is modifiers.FamilleConflit.CROISEMENT
    )
    if vru_traverse:
        level_ajuste = level
    else:
        level_ajuste = lateral.rétrograder(level, facteur, cfg.retrogradation_crans)
