...
def rétrograder(niveau, facteur: float, plafond_crans: int = 2):
    ...
    facteur < 0.4 : `plafond_crans` crans en dessous
    Le résultat est borné à SAFE.

    Le plafond par défaut de **deux crans** est une marge de sécurité
    délibérée : un CRITICAL ne peut donc pas descendre sous WATCH, même si la
    géométrie le déclare hors trajectoire. La raison est que l'écart latéral
    euclidien est une estimation peu fiable — notamment en virage.

    Quand l'écart provient des identifiants de voie CARLA (`road_id`,
    `lane_id`), il est en revanche fiable : on *sait* que le véhicule est dans
    la voie opposée, on ne le devine pas. Passer `plafond_crans=3` autorise
    alors la rétrogradation complète jusqu'à SAFE. Mesure : sans effet sur les
    24 scénarios de référence — mais aucun d'eux n'exerce ce chemin, la mesure
    ne vaut donc pas preuve de sûreté générale. C'est pourquoi le
    comportement reste désactivé par défaut.
    """
    ...
    return RiskLevel(max(int(niveau) - plafond_crans, int(RiskLevel.SAFE)))
