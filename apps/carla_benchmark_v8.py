...
def main() -> int:
    ...
    global RiskLevel 
    from risk_engine import Geometrie, RiskLevel, ScenarioContext, TypeRoute, assess_risk, commande_recommandee, ObsAgent, recommander
    ...
    print("=" * 62)
    print(f"  BENCHMARK V8 (FIX VIRAGE) - scenario {args.scenario} | TM: {profil_tm}")
    ...
    try:
        with CarlaBridge(host=args.host, port=args.port, fixed_delta_seconds=dt) as pont:
            ...
            while t_sim < duree_max:
                ...
                v_lim_ms = cr.limite_vitesse_kmh / 3.6
                ...
                # Si le verdict est SAFE, l'ACC (le script) prend le contrôle pour fluidifier la conduite
                    ...
                    elif delta_v > DEADBAND_V_MS: throttle = min(THROTTLE_MAX, delta_v / max(v_cible, 1.0))
                ...
                if temps_immobile > args.blocage_max_s:
                    print(f"\n⛔ BLOCAGE : ego immobile {temps_immobile:.1f}s (raison: {raison_arret or 'inconnue'}). Abandon.")
                    statut_fin = "bloque"
                    break
                ...
                if (distance_parcourue > args.distance * FRACTION_MIN_AVANT_ARRIVEE and pos.distance(arrivee) < TOLERANCE_ARRIVEE_M):
                    print("\n✅ POINT B ATTEINT : fin de la simulation.")
                    arrive = True; statut_fin = "arrive"
                    break

                if distance_parcourue >= args.distance:
                    print("\n✅ DISTANCE CIBLE PARCOURUE : fin de la simulation.")
                    arrive = True; statut_fin = "arrive"
                    break

            if statut_fin == "timeout": print("\n⚠ Fin par timeout (duree_max atteinte).")
        ...
        try:
            with open(resume_path, "w", newline="", encoding="utf-8") as f:
                w = csv.writer(f)
                w.writerow(["scenario", "profil_tm", "seed", "statut_fin", "arrive", "temps_arrivee_s", "vitesse_moyenne_kmh", "distance_parcourue_m", "n_collisions", "n_freinages", "niveau_max"])
                w.writerow([args.scenario, profil_tm, args.seed, statut_fin, int(arrive), f"{t_sim:.2f}", f"{vitesse_moy_kmh:.2f}", f"{distance_parcourue:.1f}", n_collisions, n_freinages, niveau_max.name])
        except Exception as e: print(f"! Ecriture du resume impossible : {e}")
