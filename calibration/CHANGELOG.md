...
## Phase 11 — v1.8 : le PET géométrique (D15)

**Contexte.** Le baseline v1.7 (0 manquée · 8 fausses alarmes · 16/24) était
défendu mais visiblement conservateur. L'analyse des 8 écarts a montré qu'ils se
répartissaient en deux familles distinctes, appelant des traitements différents :

- **5 conflits longitudinaux** (SC-05, SC-08, SC-09, SC-17, SC-22) — aucun
  problème de lecture du contexte. Le sur-classement vient des paramètres RSS
  conservateurs (`b_min`) et de la marge contextuelle multiplicative.
- **3 piétons traversants** (SC-03, SC-12, SC-13) — la métrique décisive était
  le **TTC**, appliqué hors de son domaine de validité.

**Trois pistes mesurées puis abandonnées.** Toutes violaient la contrainte de
sécurité, ce qui constitue un résultat en soi :

| Piste | Effet mesuré | Verdict |
|---|---|---|
| Plafonner la marge contextuelle | 1 manquée dès ×2,0 | rejetée |
| La plafonner pour les motorisés seuls | 2 manquées (SC-11, SC-20) | rejetée |
| Déclasser si corroboration = 1/6 | 8 des 13 vrais positifs concernés | rejetée |

Ces trois échecs établissent que **le conservatisme est structurel** : la marge
...
(piéton de nuit, verglas) et par construction excessive pour les cas doux. Les
fausses alarmes sur SC-05 et SC-22 sont le prix payé pour attraper SC-15 et
SC-21.

**Le changement retenu.** Trois modifications indissociables (voir D15) :
PET géométrique 2D, retrait du TTC pour les conflits de croisement, et
modulation du PET par la marge contextuelle. Mesures intermédiaires notables :
le PET 2D seul n'apporte rien (le PET n'était pas décisif) ; le retrait du TTC
seul produit 4 détections manquées.

**Effet.** manquées **0** (inchangé) · fausses alarmes **8 → 7** ·
exactitude **16/24 → 17/24 (71 %)**. SC-12 passe de WATCH à SAFE.

**Statut du moteur (v1.8, état courant).** manquées **0** · fausses alarmes
**7** · exactitude **17/24 (71 %)**. Configuration : `RiskConfig()` par défaut
avec `pet_2d=True`, `ttc_hors_croisement=True`, `pet_horizon_s=4.0`, filtrage
latéral 2D actif, étiquettes v1.7 **inchangées** — aucune étiquette n'a été
révisée pour faire converger le résultat.

**Les 7 écarts restants.** SC-03, SC-05, SC-08, SC-09, SC-13, SC-17, SC-22 —
tous des fausses alarmes, aucune détection manquée. Six relèvent du compromis
structurel décrit plus haut ; SC-13 (piéton à 6 m, attendu WATCH, prédit
CRITICAL) est le seul cas où le PET 2D est plus sévère que l'étiquette, la
coïncidence temporelle y étant réelle.
