...
**161 tests, 100 % au vert.** Couverture globale 56 % — mais cette moyenne mélange
...
- **test_carla_integration.py** (10) — chaîne d'intégration CARLA exécutée
  contre un **double de test** (`tests/fake_carla.py`), qui implémente assez de
  l'API CARLA pour valider signatures et enchaînement des appels sans GPU. Ces
  tests existent parce que quatre erreurs d'API sont passées en production faute
  de serveur disponible : `get_synchronous_mode` inexistant, `to_carla_weather`
  mal nommé, `contexte_courant()` appelé sans argument, et `tick()` manquant
  avant le spawn du trafic (qui faisait lire une position nulle et spawner zéro
  véhicule). Chacune aurait été détectée ici en une seconde. Ils ne valident
  PAS la physique — le double n'en a quasiment pas.
- **test_pet_2d.py** (12) — géométrie partagée moteur/simulateur, PET 2D et son
  horizon de confiance, et l'indissociabilité des deux changements de D15. Un de
  ces tests **documente une régression attendue** : retirer le TTC en croisement
  sans PET 2D doit produire des détections manquées. S'il passe à zéro, c'est que
  le moteur a changé et que D15 doit être réexaminé.
- **test_conduite_libre.py** (15) — classification blueprint → TypeAgent et
  machine à états de l'assistance (aucune intervention en mode évaluation,
  persistance de l'alerte, AEB qui ne se déclenche pas sur un CRITICAL ponctuel
  ni ne double le conducteur, désengagement sur reprise).
...
- **test_invariants.py** (12) — les propriétés que le moteur doit toujours
  ...
  inévitable = CRITICAL. Depuis l'audit (D14), deux invariants supplémentaires
  garantissent que la distance RSS croît quand l'adhérence baisse et qu'elle
  reste du même ordre que la distance d'arrêt physique sur sol dégradé.
